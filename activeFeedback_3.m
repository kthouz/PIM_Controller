...
%[cnames rnames ceditable cformat tdata] = tSettings(n,m);
intensity = .3;
opacity = 1;
camCmd = 0;
...
%blank(:,:,1) = 0; blank(:,:,2) = 250; blank(:,:,3) = 0;
%blank(:,:,:) = 1;
%blank(:,:,3) = .7;
%fprintf(num2str(height))
%fprintf(num2str(width))
fullscreen(blank,outDevice);
        ...
        set(h.quit_button,'Backgroundcolor','y');
        h.ax = axes('Parent',h.fig,'Position',[0.29 0.225 0.64 0.7],'Units','normalized');
    ...
    function snapshot_callback(source,eventdata)
        clc
        %%%This method take a snapshot from the preview camera.
        if camCmd == 0
            openCloseCam(1)
